...
instImage = pygame.image.load("Instructions.png")
cpuScreen = pygame.image.load("CPU.png")
blurImage = pygame.transform.scale(pygame.image.load("blur.png"), (2500, 1625))
buttonSound = pygame.mixer.Sound("buttonSound.wav")
buttonSound.set_volume(0.2)
pieceSound = pygame.mixer.Sound("pieceSound.wav")
errorSound = pygame.mixer.Sound("Invalid.wav")
font = pygame.font.Font("telegraphem.ttf",  120)
font2 = pygame.font.Font("telegraphem.ttf",  100)
...
# setting up bgm
pygame.mixer.music.load("connect4bgm.wav")
...
def animation(condition, colour, column, board, cpu):
    if cpu == False:
        ...
        if colour == RED: drawPiece(2, pieceX[checkCol(posX, pieceX)], False)
        else: drawPiece(1, pieceX[checkCol(posX, pieceX)], False)
        ...
        pygame.draw.circle(screen, colour, (column, condition), 50)
        pieceSound.play()
    
# function updates the board state according to the player and their move
def dropPiece(col, player, board, cpu):
    if board[0][col] == 0: # checks if the column is open
        for i in range(ROWS, 0, -1): # checks every row starting from bottom to top
            if board[i-1][col] == 0: # checks for open slots in that column
                ...
                if player == 1: animation(pieceY[i-1], RED, pieceX[col], board, cpu) # player 1
                elif player == 2: animation(pieceY[i-1], YELLOW, pieceX[col], board, cpu) # player 2
                break
    ...
    if win == True: # if checkWin is true, return dropPiece as true
        ...
        if cpu == False: # cpu off
            return loadWin(player, False)
        else: # cpu on
            return loadWin(player, True)
        ...
        for event in pygame.event.get(): # getting list of events
            ...
            if event.type == pygame.KEYDOWN: # if the user inputs esc key, the game will unpause and will reset the board back to normal.
                if event.key == pygame.K_ESCAPE:
                    ...
                    posX = pygame.mouse.get_pos()[0] # takes only the x-coordinate of the mouse
                    if player == 1: drawPiece(1, pieceX[checkCol(posX, pieceX)], False)
                    else: drawPiece(2, pieceX[checkCol(posX, pieceX)], False)
                    paused = False
...
# Function shows the cpu selection screen
def show_cpu_selection():
    screen.blit(cpuScreen,(0,0))
    pygame.display.flip()
    running = True
    while running:
        for event in pygame.event.get(): # getting list of events
            ...
            if event.type == pygame.KEYDOWN and event.key == pygame.K_ESCAPE: # allow user to use esc to go previous screen
                buttonSound.play()
                loadMenu()
                running = False
                return False
            ...
            if event.type == pygame.MOUSEBUTTONDOWN and event.button == 1: # left click
                ...
                if posY >= (HEIGHT-280)/2 and posY <= (HEIGHT-280)/2 + 280: # when the mouse is on this specific y level
                    if posX >= 0 and posX <= 423: # this button is for cpu on
                        buttonSound.play()
                        return 1
                    elif posX >= 437 and posX <= 860: # this button is for cpu off
                        buttonSound.play()
                        return 2
                elif (posX >= 0 and posX <= 80) and (posY >= 0 and posY <= 80): # back button
                    running = False
                    return False
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.MOUSEBUTTONDOWN and event.button == 1:
                ...
                if (posX >= (WIDTH-282)/2 and posX <= (WIDTH-282)/2 + 282) and (posY >= 180 and posY <= 300): # play button
                    buttonSound.play()
                    cpu = show_cpu_selection()
                    if cpu == False:
                        loadMenu()
                    else:
                        running = playGame(cpu)
    ...
    while not gameOver: # while the game is running
        ...
        for event in pygame.event.get(): # allows user to quit during the game
            ...
            if event.type == pygame.MOUSEMOTION or event.type == pygame.MOUSEBUTTONUP: # when the mouse moves or after a mouse click is registered
                ...
                if cpu == 2:
                    drawPiece(player, pieceX[checkCol(posX, pieceX)], False) # the piece indicator at the top of the screen will redraw itself
                else:
                    drawPiece(1, pieceX[checkCol(posX, pieceX)], True)
            ...
            if event.type == pygame.MOUSEBUTTONDOWN and event.button == 1: # upon left clicking
                ...
                if cpu == 2: # cpu off
                    if player == 1: # player 1's turn
                        gameOver = dropPiece(col, 1, board, False) # play their turn

                        if gameOver == None: # if turn is invalid, replay.
                            gameOver = dropPiece(col, 1, board, False)
                            ...
                            if turns == 42: # checks for draw
                                gameOver = loadWin(3, False)
                    
                    else: # player 2's turn
                        gameOver = dropPiece(col, 2, board, False)
                        ...
                        if gameOver == None:
                            ...
                            if turns == 42:
                                gameOver = loadWin(3, False)
                else: # cpu on || It was still quite work in progress during creation, there are a couple bugs in cpu mode however player vs player works flawlessly.
                    player = 1 # force player to be first
                    playerTurn = True # player turn starts off true
                    while playerTurn:
                        gameOver = dropPiece(col, 1, board, True) # play user's turn

                        if gameOver == "restart": # If user clicks restart
                            gameOver = False
                            turns = 0
                            board = createBoard()
                            screen.fill(GRAY)
                            ...
                            playerTurn = False
                            if turns == 42: # checks for draw
                                gameOver = loadWin(3, True)
                    
                    else: # when playerTurn is false
                        tempBool = cpuTurn(gameOver, 2, board, turns) # use the temporary boolean to avoid accidentally crashing the game at the end and ensure playerturn has priority.
                        if tempBool == False:
                            playerTurn = True
                        else:
                            playerTurn = True
...
#cpu turn function
def cpuTurn(bool, turn, board, turnNum):
    rng = random.randint(0,6) # random number generator because it is a random cpu
    bool = dropPiece(rng, turn, board, True) # similar to player's play

    while bool == None: # while bool is none the cpu will constantly replay
        rng = random.randint(0,6)
        bool = dropPiece(rng, turn, board, True)
    
    else: # otherwise, the cpu will play the turn and update the turn counter
        turnNum += 1
        if turnNum == 42: # in the event of a draw
            bool = loadWin(3, True)
        
    return bool # return the boolean statement from all previous functions
